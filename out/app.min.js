/*! coffeescript-snake - v1.0.0 - 2017-09-15 *//*! jQuery v3.2.1 | (c) JS Foundation and other contributors | jquery.org/license */
...
;(function() {
  "use strict";
  angular.module("app", []).controller("DefaultController", function($scope, $timeout, $interval) {
    var HEIGHT, Position, Snake, WIDTH, colorMouse, colorSnake, defineKeyboardControls, displayMessage, getSquare, initializeSnake, isMouseFound, isPositionValid, move, paintBoard, reset, resetBoard, setSquare, showMouse, start;
    WIDTH = 40;
    HEIGHT = 20;
    Snake = function(length, positions, direction) {
      this.length = length;
    ...
    resetBoard = function() {
      var _, col_ndx, i, j, len, len1, ref, ref1, row_ndx;
      $scope.board = new Array(HEIGHT);
      ref = $scope.board;
      for (row_ndx = i = 0, len = ref.length; i < len; row_ndx = ++i) {
        _ = ref[row_ndx];
        $scope.board[row_ndx] = new Array(WIDTH);
        ref1 = $scope.board[row_ndx];
        for (col_ndx = j = 0, len1 = ref1.length; j < len1; col_ndx = ++j) {
          _ = ref1[col_ndx];
    ...
    setSquare = function(row_ndx, col_ndx, color) {
      var board;
      board = $scope.board;
    ...
    getSquare = function(color) {
      return "<div class=\"square-" + color + "\"></div>";
    };
    paintBoard = function() {
      var _, board, board_html, col_ndx, i, j, len, len1, ref, row_ndx;
      board = $scope.board;
      board_html = "";
      for (row_ndx = i = 0, len = board.length; i < len; row_ndx = ++i) {
        _ = board[row_ndx];
        ref = board[row_ndx];
        for (col_ndx = j = 0, len1 = ref.length; j < len1; col_ndx = ++j) {
          _ = ref[col_ndx];
          board_html += board[row_ndx][col_ndx];
        }
      }
      $timeout(function() {
        return $("div#board").html(board_html);
      });
      return void 0;
    };
    ...
    colorSnake = function() {
      var i, len, ndx, position, ref, snake;
      snake = $scope.snake;
      ref = snake.positions;
      for (ndx = i = 0, len = ref.length; i < len; ndx = ++i) {
        position = ref[ndx];
        if (ndx === 0) {
    ...
    isPositionValid = function(row, col) {
      var i, isValid, len, position, ref;
      isValid = row >= 0 && row < $scope.board.length && col >= 0 && col < $scope.board[0].length;
      if (isValid) {
        ref = $scope.snake.positions;
    ...
    isMouseFound = function(row, col) {
      return row === $scope.mouse.row && col === $scope.mouse.col;
    };
    move = function() {
      var col, row, snake;
      if (!$scope.isPaused) {
    ...
    showMouse = function() {
      var col, i, index, indices, indices_to_exclude, j, k, len, position, ref, ref1, ref2, row;
      indices_to_exclude = [];
      ref = $scope.snake.positions;
      for (i = 0, len = ref.length; i < len; i++) {
        position = ref[i];
      ...
      indices = [];
      for (row = j = 0, ref1 = HEIGHT - 1; 0 <= ref1 ? j < ref1 : j > ref1; row = 0 <= ref1 ? ++j : --j) {
        for (col = k = 0, ref2 = WIDTH - 1; 0 <= ref2 ? k < ref2 : k > ref2; col = 0 <= ref2 ? ++k : --k) {
          index = row * WIDTH + col;
          if (indices_to_exclude.indexOf(index) === -1) {
            indices.push(index);
          }
        }
      }
    ...
    defineKeyboardControls = function() {
      var preventDefault;
      ...
      document.onkeydown = function(e) {
        ...
        if ($scope.isStarted) {
          if (!$scope.isPaused) {
            if (e.keyCode === 38) {
              if ($scope.snake.direction !== "S" || $scope.snake.positions.length === 1) {
                $scope.snake.direction = "N";
              }
            } else if (e.keyCode === 40) {
              if ($scope.snake.direction !== "N" || $scope.snake.positions.length === 1) {
                $scope.snake.direction = "S";
              }
            } else if (e.keyCode === 37) {
              if ($scope.snake.direction !== "E" || $scope.snake.positions.length === 1) {
                $scope.snake.direction = "W";
              }
            } else if (e.keyCode === 39) {
              if ($scope.snake.direction !== "W" || $scope.snake.positions.length === 1) {
                $scope.snake.direction = "E";
              }
            }
          }
          if (e.keyCode === 80 || e.keyCode === 13) {
            return $scope.isPaused = !$scope.isPaused;
